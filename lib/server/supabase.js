...
const getSupabaseUrl = () => envValue('SUPABASE_URL') || envValue('VITE_SUPABASE_URL');
...
const getSupabasePublishableKey = () =>
  ...
  envValue('VITE_SUPABASE_PUBLISHABLE_DEFAULT_KEY');

const createNonPersistedClient = (supabaseUrl, key) =>
...
export const getSupabaseServerClient = () => {
  ...
  if (!supabaseServerClient) {
    supabaseServerClient = createNonPersistedClient(supabaseUrl, serviceRoleKey);
  }
...
export const getSupabaseAuthClient = () => {
  const supabaseUrl = getSupabaseUrl();
  const authKey = getSupabasePublishableKey() || envValue('SUPABASE_SERVICE_ROLE_KEY');

  if (!supabaseUrl || !authKey) {
    throw new Error('Missing SUPABASE_URL and publishable key for server-side user auth.');
  }

  if (!supabaseAuthClient) {
    supabaseAuthClient = createNonPersistedClient(supabaseUrl, authKey);
  }
...
export const ensureSupabaseUserCanPasswordLogin = async (email) => {
